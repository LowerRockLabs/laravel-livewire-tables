    ...
    @endif
        <div @class(['form-check' => $isBootstrap])>
            <input id="{{ $dataTableFingerprint }}-filter-{{ $filter->getKey() }}-select-all{{ $filter->hasCustomPosition() ? '-'.$filter->getCustomPosition() : null }}" wire:input="selectAllFilterOptions('{{ $filter->getKey() }}')" {{ 
                    ...
                    ->class($isBootstrap ? [
                        'form-check-input' => ($filterInputAttributes['default-styling'] ?? true),
                    ] : [])
        ...
        @foreach($filter->getOptions() as $key => $value)
            <div @class([
                ...
                wire:key="{{ $dataTableFingerprint }}-filter-{{ $filter->getKey() }}-{{ $loop->index }}{{ $filter->hasCustomPosition() ? '-'.$filter->getCustomPosition() : null }}" value="{{ $key }}" {{ 
                    ...
                    ->class($isTailwind4 ? [
                        'tw4ph rounded shadow-sm transition duration-150 ease-in-out focus:ring focus:ring-opacity-50 disabled:opacity-50 disabled:cursor-wait' => ($filterInputAttributes['default-styling'] ?? true),
                        'tw4ph text-indigo-600 border-gray-300 focus:border-indigo-300  focus:ring-indigo-200  dark:bg-gray-900 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600 dark:focus:bg-gray-600' => ($filterInputAttributes['default-colors'] ?? true),
                        ] : [])
                    ->class($isBootstrap ? [
                        // Bootstrap
                        'form-check-input' => ($filterInputAttributes['default-styling'] ?? true),
