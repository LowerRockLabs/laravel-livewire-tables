...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('tableContextMenu', (contextMenuIsEnabled) => ({
        contextMenuOpen: false,
        contextRowPrimaryKey: '',
        contextMenuToggle: function(event) {
            let rowPk = event.target.closest('tr').getAttribute('rowpk');
            
            if (rowPk === undefined || rowPk === null) {
                this.contextMenuOpen = false;
                return;
            }
            this.contextRowPrimaryKey = rowPk;
